...
contract AppaVillage is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("Appa Village NFT", "APPA")  {
        setBaseURI(baseURI);

        // team gets the first AppaVillage
        _safeMint(a1, 0);
        _safeMint(a2, 1);
        _safeMint(a1, 2);
        _safeMint(a1, 3);
    ...
    function getAppa (uint256 num) public payable {
        uint256 supply = totalSupply();
        require( !_paused,                              "Sale paused" );
        require( num < 21,                              "You can adopt a maximum of 20 Appa" );
        require( supply + num < maxSupply - _reserved,      "Exceeds maximum Appas supply" );
    ...
    function setHide(bool _value) public onlyOwner {
        _hide = _value;
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns(string memory) {
        if(_hide) {
            return "https://gateway.pinata.cloud/ipfs/QmW3e63rtS2mHT4JS4LT6zFYapHdzEKxEatCzniQz4xkiX/1.json";
        } 
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= _reserved, "Exceeds reserved Appa supply" );
    ...
    function pause(bool val, uint8 _type) public onlyOwner {
        /** 
         * @dev 
         * type = 1 : public-sale
         * - false = played
         * - true = paused
         * type = 2 : pre-sale pause-play 
         * - false = played
         * - true = paused
        */
        if(_type == 1) {
    ...
    function claimWhitelist(uint256 num) public payable {
        uint256 supply = totalSupply();
        require(!_presalePaused, 'presale ended!');
        require(num < 4, 'max 3 appa per wallet');
        require(isWhitelisted(msg.sender), 'this address is not in the whitelist');
        require(totalSupply() < maxSupply - _reserved, 'all tokens are minted!');
        require(balanceOf(msg.sender) < 3, 'only 3 nfts per address');
        require( msg.value >= _price * num, "Ether sent is not correct" );
        for(uint256 i; i < num; i++) {
            _safeMint( msg.sender, supply + i );
        }
